...
# Mode kualitas validasi email:
# fast     = filter lokal + MX saja
# accurate = filter lokal + MX + Verifalia jika API tersedia
# clean    = filter paling ketat, blokir free email dan role/noise agresif
DEFAULT_QUALITY_MODE   = "accurate"
QUALITY_MODES          = {"fast", "accurate", "clean"}
...
# Runtime / fitur tambahan
BLACKLIST_DIR = "blacklists"
os.makedirs(BLACKLIST_DIR, exist_ok=True)
CUSTOM_BLOCKED_EMAILS_FILE    = os.path.join(BLACKLIST_DIR, "blocked_emails.txt")
CUSTOM_BLOCKED_DOMAINS_FILE   = os.path.join(BLACKLIST_DIR, "blocked_domains.txt")
CUSTOM_BLOCKED_KEYWORDS_FILE  = os.path.join(BLACKLIST_DIR, "blocked_keywords.txt")
LAST_JOB_STATE_FILE           = os.path.join(OUTPUT_DIR, "last_job_state.json")

ACTIVE_JOBS: dict = {}
CANCELLED_JOBS: set = set()
CUSTOM_BLACKLIST_CACHE: dict = {"mtime": 0, "emails": set(), "domains": set(), "keywords": set()}
...
def read_lines_file(path: str) -> set[str]:
    """Baca file blacklist custom. Baris kosong dan komentar # diabaikan."""
    ...
    try:
        ...
        logging.warning(f"Gagal baca blacklist {path}: {e}")
        return set()


def get_custom_blacklists(force_reload: bool = False) -> dict:
    """Cache blacklist custom agar tidak baca file berulang-ulang."""
    files = [CUSTOM_BLOCKED_EMAILS_FILE, CUSTOM_BLOCKED_DOMAINS_FILE, CUSTOM_BLOCKED_KEYWORDS_FILE]
    latest_mtime = 0
    for path in files:
        if os.path.exists(path):
            latest_mtime = max(latest_mtime, os.path.getmtime(path))

    if (not force_reload) and CUSTOM_BLACKLIST_CACHE.get("mtime") == latest_mtime:
        return CUSTOM_BLACKLIST_CACHE

    CUSTOM_BLACKLIST_CACHE["mtime"] = latest_mtime
    CUSTOM_BLACKLIST_CACHE["emails"] = read_lines_file(CUSTOM_BLOCKED_EMAILS_FILE)
    CUSTOM_BLACKLIST_CACHE["domains"] = read_lines_file(CUSTOM_BLOCKED_DOMAINS_FILE)
    CUSTOM_BLACKLIST_CACHE["keywords"] = read_lines_file(CUSTOM_BLOCKED_KEYWORDS_FILE)
    return CUSTOM_BLACKLIST_CACHE


def ensure_blacklist_files() -> None:
    samples = {
        CUSTOM_BLOCKED_EMAILS_FILE: "# satu email per baris\n# contoh: spam@example.com\n",
        CUSTOM_BLOCKED_DOMAINS_FILE: "# satu domain per baris\n# contoh: example.com\n# contoh suffix: .go.id\n",
        CUSTOM_BLOCKED_KEYWORDS_FILE: "# satu keyword per baris\n# contoh: rektor\n# contoh: kampus\n",
    }
    for path, content in samples.items():
        if not os.path.exists(path):
            try:
                ...
                logging.warning(f"Gagal membuat template blacklist {path}: {e}")
...
def progress_footer(start_time: float, percent: int) -> str:
    elapsed = time.time() - start_time
    if percent <= 0:
        eta = "menghitung..."
    else:
        estimated_total = elapsed / (percent / 100)
        eta = format_duration(max(0, estimated_total - elapsed))
    return f"⏱️ Durasi: {format_duration(elapsed)} | ETA: {eta}"


def sanitize_telegram_text(text: str, max_len: int = 3900) -> str:
    text = str(text)
    return text[:max_len] + "\n\n..." if len(text) > max_len else text


def job_key_from_message(message) -> str:
    try:
...
def cancel_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("⛔ Stop Scraping", callback_data="stop_scrape")]])


def is_job_cancelled(message) -> bool:
    key = job_key_from_message(message)
    return bool(key and key in CANCELLED_JOBS)


def register_job(message, mode: str, input_text: str, target: int, output_filename: str, quality_mode: str) -> str:
    key = job_key_from_message(message)
    if key:
        ACTIVE_JOBS[key] = {
            "mode": mode,
...
def update_job_rows(message, rows: list) -> None:
    key = job_key_from_message(message)
    if key in ACTIVE_JOBS:
        ACTIVE_JOBS[key]["rows"] = list(rows)


def update_job_error(message, error_text: str) -> None:
    key = job_key_from_message(message)
    if key in ACTIVE_JOBS:
        ACTIVE_JOBS[key]["last_error"] = str(error_text)[:500]


def finish_job(message) -> None:
    key = job_key_from_message(message)
    if key:
...
def save_last_job_state(mode: str, input_text: str, target: int, output_filename: str, quality_mode: str) -> None:
    try:
        payload = {
            "mode": mode,
            "input": input_text,
        ...
        with open(LAST_JOB_STATE_FILE, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logging.warning(f"Gagal simpan resume state: {e}")


def load_last_job_state() -> dict | None:
    try:
        if not os.path.exists(LAST_JOB_STATE_FILE):
            return None
        with open(LAST_JOB_STATE_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        logging.warning(f"Gagal baca resume state: {e}")
        return None


def get_input_prompt(mode: str) -> str:
    return {
        "web":       "🌐 Kirim URL website.\nContoh: `https://example.com | 300 | accurate`",
        "maps":      "🗺️ Kirim keyword + lokasi.\nContoh: `digital agency Jakarta | 200 | clean`",
        "dork":      "🔍 Kirim keyword + lokasi.\nContoh: `digital agency Jakarta | 500 | fast`",
        "directory": "📋 Kirim keyword + lokasi.\nContoh: `digital agency Jakarta | 300 | accurate`",
        "batch":     "📦 Kirim file .txt atau ketik URL langsung 1 per baris.\nOpsional baris: `TARGET: 300` dan `QUALITY: clean`",
    }.get(mode, "Kirim input:")


def infer_xlsx_type(columns: list[str]) -> str:
    cols = set(columns)
    for scraping_type in ("maps", "dork", "directory", "web"):
        if set(get_columns(scraping_type)).issubset(cols):
            return scraping_type
    return "unknown"


def clean_xlsx_file(path: str) -> tuple[int, int]:
    ...
    if "Email" in df.columns:
        ...
        raise ValueError("File tidak punya kolom Email")
    if "No" in df.columns:
...
BLOCKED_LOCAL_PARTS = {
    ...
    "rektorat", "dekan", "dekanat", "wakildekan", "warek", "wr", "kajur", "kaprodi",
}
...
STRICT_BLOCKED_DOMAIN_KEYWORDS = (
    ...
    "sakura.ne.jp", "rektor", "rektorat", "kampus", "universitas", "fakultas", "unair.ac.id",
...
ROLE_EMAIL_LOCAL_PARTS = {
    ...
    "kampus", "universitas", "fakultas", "dosen", "mahasiswa", "mahasiswi",
    "dekan", "dekanat", "wakildekan", "warek", "kajur", "kaprodi", "baa", "baak"
}
...
def validate_email(email: str) -> str:
    ...
    custom_bl = get_custom_blacklists()
    if email in custom_bl.get("emails", set()):
        return "Invalid"
    if domain in custom_bl.get("domains", set()):
        return "Invalid"
    if any(domain.endswith(d) for d in custom_bl.get("domains", set()) if d.startswith(".")):
        return "Invalid"
    if any(kw and kw in email for kw in custom_bl.get("keywords", set())):
        return "Invalid"
...
def is_clean_mode_blocked(email: str) -> bool:
    """Mode clean: hanya ambil email bisnis/corporate yang lebih relevan."""
    domain = get_email_domain(email)
    local = clean_email(email).split("@", 1)[0] if "@" in email else ""
    normalized = local.replace(".", "").replace("_", "").replace("-", "")
    if is_free_email_domain(domain):
        return True
    if local not in GOOD_BUSINESS_LOCAL_PARTS and normalized not in GOOD_BUSINESS_LOCAL_PARTS:
        # bukan role bisnis prioritas, tetap boleh jika bukan noise dan domain corporate
        if local in ROLE_EMAIL_LOCAL_PARTS or normalized in ROLE_EMAIL_LOCAL_PARTS:
            return True
    return False
...
def has_mx_record(domain: str) -> bool:
    ...
    try:
        ...
        logging.debug(f"MX check gagal untuk {domain}: {e}")
...
async def is_email_allowed_by_checker(
    session: aiohttp.ClientSession, email: str, quality_mode: str = DEFAULT_QUALITY_MODE
) -> tuple[bool, str]:
    email = clean_email(email)
    email_domain = get_email_domain(email)

    if quality_mode == "clean" and is_clean_mode_blocked(email):
        return False, "Clean Mode Blocked"

    mx_ok = await asyncio.to_thread(has_mx_record, email_domain)
    if not mx_ok:
        return False, "No MX"

    if quality_mode == "fast":
        return True, "Local+MX Fast"
    ...
    if VERIFALIA_ACCEPT_RISKY and status == "Risky" and quality_mode != "clean":
        return True, "Risky"
    return False, status
...
def list_xlsx_files(scraping_type: str | None = None) -> list[dict]:
    ...
    required_columns = set(get_columns(scraping_type)) if scraping_type else None
    ...
    for filename in sorted(os.listdir(OUTPUT_DIR)):
        ...
        try:
            ...
            try:
                ...
                df.columns = [str(c).strip() for c in df.columns]
                rows_count = len(df)
                file_type = infer_xlsx_type(list(df.columns))
                valid_format = "Email" in df.columns
            ...
            files.append({
                ...
                "file_type": file_type, "valid_format": valid_format,
            })
        except Exception as e:
            logging.warning(f"Gagal baca XLSX {filename}: {e}")
            continue
    return files
...
def load_existing_master(
    ...
    try:
        ...
        if "Email" not in df.columns:
            logging.warning(f"File {master_path} tidak punya kolom Email. File akan di-reset sesuai format {scraping_type}.")
            empty_df = pd.DataFrame(columns=columns)
            empty_df.to_excel(master_path, index=False)
            return empty_df, set(), set(), master_path
        ...
        logging.warning(f"Gagal baca master {master_path}: {e}. File akan di-reset.")
        empty_df = pd.DataFrame(columns=columns)
        try:
            empty_df.to_excel(master_path, index=False)
        except Exception:
...
def main_menu() -> InlineKeyboardMarkup:
    keyboard = [
        ...
        [
            InlineKeyboardButton("▶️ Resume Terakhir", callback_data="resume_last"),
            InlineKeyboardButton("🧱 Blacklist", callback_data="manage_blacklist"),
        ],
        [InlineKeyboardButton("🎯 Info Mode Kualitas", callback_data="quality_info")],
    ]
...
def manage_xlsx_menu() -> InlineKeyboardMarkup:
    keyboard = [
        ...
        [
            InlineKeyboardButton("📤 Kirim File", callback_data="list_send_xlsx:all"),
            InlineKeyboardButton("🧹 Bersihkan Duplikat", callback_data="list_clean_xlsx:all"),
        ],
        [
            InlineKeyboardButton("🔎 Cek Format", callback_data="list_check_xlsx:all"),
...
async def safe_edit_message(message, text: str, reply_markup=None) -> None:
    ...
    key = job_key_from_message(message)
    if reply_markup is None and key in ACTIVE_JOBS and not text.startswith(("✅", "⛔", "⚠️")):
    ...
    try:
        ...
        if "message is not modified" in err:
            return
        logging.warning(f"Gagal edit pesan Telegram: {e}")
    except Exception as e:
        logging.warning(f"Gagal edit pesan Telegram: {e}")


async def safe_query_edit(query, text: str, reply_markup=None, parse_mode=None) -> None:
    text = sanitize_telegram_text(text)
    try:
        ...
        if "message is not modified" in err:
            return
        logging.warning(f"Gagal edit callback: {e}")
        try:
            await query.message.reply_text(f"⚠️ Info bot: gagal update menu. Detail: {e}")
        except Exception:
            pass
    except Exception as e:
        logging.warning(f"Gagal edit callback: {e}")
...
def parse_target_from_input(text: str) -> tuple[str, int, str]:
    parts = [p.strip() for p in (text or "").split("|")]
    main_input = parts[0] if parts else ""
    target = DEFAULT_TARGET_EMAILS
    quality_mode = DEFAULT_QUALITY_MODE

    for part in parts[1:]:
        low = part.lower().replace("quality:", "").replace("mode:", "").strip()
        if low in QUALITY_MODES:
            quality_mode = low
            continue
        try:
            target = max(1, min(int(part.strip()), 10000))
        except ValueError:
            pass

    return main_input.strip(), target, quality_mode
...
async def process_email(
    ...
    allowed, _ = await is_email_allowed_by_checker(session, email, quality_mode)
    if not allowed:
        counters["invalid"] += 1
        return False
...
async def scrape_website(
    ...
    async with aiohttp.ClientSession() as session:
        while queued_urls:
            if is_job_cancelled(progress_message):
                stop_reason = "Dihentikan user"
                break
            ...
            if html:
                for email, status in extract_emails_from_text(html):
                    accepted = await process_email(
                        ...
                        found_email_set, found_domain_set, counters, quality_mode=quality_mode
                    )
                    if accepted:
                        ...
                        update_job_rows(progress_message, rows)
                ...
                failed_pages += 1
                update_job_error(progress_message, error or "Fetch gagal")
            ...
            if scanned_pages == 1 or scanned_pages % PROGRESS_UPDATE_EVERY == 0 or not queued_urls:
                await safe_edit_message(progress_message,
                    ...
                    f"{make_progress_bar(pct)} {pct}%\n{progress_footer(start_time, pct)}\n\n"
        ...
        for url in urls:
            ...
            for email, status, src_url in extracted:
                ...
                if accepted:
                    ...
                    latest_preview.insert(0, f"✅ {email} [{lbl}]")
                    latest_preview = latest_preview[:10]
                    update_job_rows(progress_message, all_rows)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "stop_scrape":
        key = job_key_from_message(query.message)
        if key and key in ACTIVE_JOBS:
            CANCELLED_JOBS.add(key)
            await safe_query_edit(
                query,
                "⛔ Permintaan stop diterima. Bot akan berhenti setelah proses halaman saat ini selesai.",
            )
        else:
            await safe_query_edit(query, "Tidak ada proses scraping aktif di pesan ini.", reply_markup=main_menu())
        return

    if data == "back_main":
        context.user_data.clear()
        await safe_query_edit(query, "Pilih mode scraping:", reply_markup=main_menu())
        return
    ...
    if data == "quality_info":
        await safe_query_edit(
            query,
            "🎯 Mode kualitas email:\n\n"
            "⚡ fast = filter lokal + MX, paling cepat.\n"
            "🎯 accurate = filter lokal + MX + Verifalia jika tersedia.\n"
            "🧹 clean = paling ketat, blokir free email dan role/noise lebih agresif.\n\n"
            "Format input:\n"
            "`digital agency Jakarta | 300 | clean`\n"
            "`https://example.com | 200 | fast`",
            reply_markup=main_menu(),
            parse_mode="Markdown",
        )
        return

    if data == "manage_blacklist":
        ensure_blacklist_files()
        bl = get_custom_blacklists(force_reload=True)
        await safe_query_edit(
            query,
            "🧱 Blacklist custom aktif.\n\n"
            f"📧 Emails: {len(bl.get('emails', set()))}\n"
            f"🌐 Domains: {len(bl.get('domains', set()))}\n"
            f"🔑 Keywords: {len(bl.get('keywords', set()))}\n\n"
            "Edit file ini di folder project:\n"
            f"- `{CUSTOM_BLOCKED_EMAILS_FILE}`\n"
            f"- `{CUSTOM_BLOCKED_DOMAINS_FILE}`\n"
            f"- `{CUSTOM_BLOCKED_KEYWORDS_FILE}`\n\n"
            "Satu item per baris. Baris diawali # akan diabaikan.",
            reply_markup=main_menu(),
            parse_mode="Markdown",
        )
        return
    ...
    if data == "resume_last":
        state = load_last_job_state()
        if not state:
            await safe_query_edit(query, "Belum ada scraping terakhir untuk dilanjutkan.", reply_markup=main_menu())
            return
        mode = state.get("mode", "web")
        context.user_data.clear()
        context.user_data["mode"] = "web" if mode == "batch" else mode
        context.user_data["actual_mode"] = mode
        context.user_data[f"selected_{context.user_data['mode']}_xlsx"] = state.get("output_filename")
        context.user_data["resume_prefill"] = state
        await safe_query_edit(
            query,
            "▶️ Resume terakhir ditemukan.\n\n"
            f"Mode: {mode}\n"
            f"Input: {state.get('input')}\n"
            f"Target: {state.get('target')}\n"
            f"Quality: {state.get('quality_mode')}\n"
            f"Output: {state.get('output_filename')}\n\n"
            "Kirim ulang input yang sama, atau copy ini:\n"
            f"`{state.get('input')} | {state.get('target')} | {state.get('quality_mode')}`",
            reply_markup=main_menu(),
            parse_mode="Markdown",
        )
        return
    ...
    if data == "manage_xlsx":
        await safe_query_edit(query, "📁 Kelola file XLSX hasil scraping.", reply_markup=manage_xlsx_menu())
        return
    ...
    if data.startswith("list_xlsx:"):
        ...
        for i, f in enumerate(files[:20], 1):
            ok = "✅" if f.get("valid_format") else "⚠️"
            lines.append(f"{i}. {ok} {f['filename']}\n   Email: {f['rows']} | {f['size_kb']:.1f} KB | {f['modified']}")
        await safe_query_edit(query, "\n".join(lines), reply_markup=manage_xlsx_menu())
        return

    if data.startswith("list_send_xlsx:") or data.startswith("list_clean_xlsx:") or data.startswith("list_check_xlsx:"):
        action = data.split(":", 1)[0].replace("list_", "")
        files = list_xlsx_files(None)
        context.user_data[f"{action}_files_all"] = files
        if not files:
            await safe_query_edit(query, "Belum ada file XLSX.", reply_markup=manage_xlsx_menu())
            return
        labels = {"send_xlsx": "📤 Pilih file untuk dikirim:", "clean_xlsx": "🧹 Pilih file untuk dibersihkan:", "check_xlsx": "🔎 Pilih file untuk dicek:"}
        await safe_query_edit(query, labels.get(action, "Pilih file:"), reply_markup=build_xlsx_file_menu(files, action, "all"))
        return

    if data.startswith("send_xlsx:") or data.startswith("clean_xlsx:") or data.startswith("check_xlsx:"):
        action, scraping_type, idx_text = data.split(":")
        files = context.user_data.get(f"{action}_files_{scraping_type}") or context.user_data.get(f"{action}_files_all") or list_xlsx_files(None)
        try:
            f_info = files[int(idx_text)]
        except Exception:
            await safe_query_edit(query, "File tidak ditemukan.", reply_markup=manage_xlsx_menu())
            return
        path = f_info["path"]
        if not (os.path.dirname(os.path.abspath(path)) == os.path.abspath(OUTPUT_DIR) and path.lower().endswith(".xlsx")):
            await safe_query_edit(query, "File tidak aman untuk diproses.", reply_markup=manage_xlsx_menu())
            return
        if action == "send_xlsx":
            await query.message.reply_document(document=open(path, "rb"), filename=os.path.basename(path), caption="📤 File XLSX")
            await safe_query_edit(query, "File sudah dikirim.", reply_markup=manage_xlsx_menu())
            return
        if action == "clean_xlsx":
            try:
                before, after = clean_xlsx_file(path)
                await safe_query_edit(query, f"🧹 Selesai bersihkan duplikat.\nFile: {f_info['filename']}\nSebelum: {before}\nSesudah: {after}", reply_markup=manage_xlsx_menu())
            except Exception as e:
                await safe_query_edit(query, f"Gagal bersihkan file:\n{e}", reply_markup=manage_xlsx_menu())
            return
        if action == "check_xlsx":
            try:
                df = pd.read_excel(path, nrows=5)
                cols = [str(c).strip() for c in df.columns]
                ftype = infer_xlsx_type(cols)
                missing_email = "Email" not in cols
                await safe_query_edit(query, "🔎 Hasil cek format XLSX:\n\n" +
                    f"File: {f_info['filename']}\n" +
                    f"Tipe terdeteksi: {ftype}\n" +
                    f"Kolom Email: {'❌ tidak ada' if missing_email else '✅ ada'}\n" +
                    "Kolom:\n- " + "\n- ".join(cols), reply_markup=manage_xlsx_menu())
            except Exception as e:
                await safe_query_edit(query, f"Gagal cek file:\n{e}", reply_markup=manage_xlsx_menu())
            return
    ...
    if data == "do_delete_xlsx":
        ...
        if not f_info:
            await safe_query_edit(query, "Tidak ada file yang dipilih.", reply_markup=manage_xlsx_menu())
            return
        try:
            path = f_info["path"]
            if os.path.dirname(os.path.abspath(path)) == os.path.abspath(OUTPUT_DIR) and path.lower().endswith(".xlsx"):
                os.remove(path)
                await safe_query_edit(query, f"✅ File dihapus:\n{f_info['filename']}", reply_markup=manage_xlsx_menu())
            else:
                await safe_query_edit(query, "File tidak aman untuk dihapus.", reply_markup=manage_xlsx_menu())
        except Exception as e:
            await safe_query_edit(query, f"Gagal hapus file:\n{e}", reply_markup=manage_xlsx_menu())
        return

    mode_map = [
        ("scrape_web",       "web",       "🌐 Scraping Web\n\nFormat: `https://example.com | 300 | accurate`\n\nPilih output XLSX:"),
        ("scrape_maps",      "maps",      "🗺️ Scraping Google Maps\n\nFormat: `digital agency Jakarta | 200 | clean`\n\nPilih output XLSX:"),
        ("scrape_dork",      "dork",      "🔍 Google Dorking\n\nFormat: `digital agency Jakarta | 500 | fast`\n\nPilih output XLSX:"),
        ("scrape_directory", "directory", "📋 Scraping Direktori Bisnis\n\nFormat: `digital agency Jakarta | 300 | accurate`\n\nPilih output XLSX:"),
    ...
    for mode_key, label, prompt in mode_map:
        if data == mode_key:
            ...
            await safe_query_edit(query, prompt, reply_markup=output_choice_menu(label), parse_mode="Markdown")
            return
    ...
    if data.startswith("xlsx_new:"):
        ...
        await safe_query_edit(query, "🆕 Kirim nama file XLSX baru.\nContoh: `leads_jakarta.xlsx`", parse_mode="Markdown")
        return
    ...
    if data.startswith("xlsx_old:"):
        ...
        if not files:
            await safe_query_edit(query, f"Belum ada XLSX lama untuk {scraping_type.upper()}. Buat file baru dulu.", reply_markup=output_choice_menu(scraping_type))
            return
        await safe_query_edit(query, f"📂 Pilih XLSX lama untuk output {scraping_type.upper()}:", reply_markup=build_xlsx_file_menu(files, "select_xlsx", scraping_type))
        return
    ...
    if data.startswith("select_xlsx:"):
        ...
        try:
            ...
            await safe_query_edit(query, "File tidak ditemukan.", reply_markup=output_choice_menu(scraping_type))
            return
        ...
        await safe_query_edit(query, f"✅ Output: {f_info['filename']}\n\n" + get_input_prompt(actual_mode), parse_mode="Markdown")
        return
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("awaiting_new_xlsx_name"):
        ...
        if os.path.exists(filepath):
            await update.message.reply_text("Nama file sudah ada. Kirim nama berbeda.", reply_markup=output_choice_menu(scraping_type))
    ...
    if update.message.document:
        ...
        if (doc.file_name or "").lower().endswith(".txt") and actual_mode == "batch":
            ...
            target, urls, quality_mode = DEFAULT_TARGET_EMAILS, [], DEFAULT_QUALITY_MODE
            for line in lines:
                low = line.lower()
                if low.startswith("target:"):
                    try:
                        target = max(1, min(int(line.split(":", 1)[1].strip()), 10000))
                    except ValueError:
                        pass
                elif low.startswith("quality:") or low.startswith("mode:"):
                    q = line.split(":", 1)[1].strip().lower()
                    if q in QUALITY_MODES:
                        quality_mode = q
            ...
            await run_scraping_and_send(update, context, scrape_batch, "web", actual_mode, f"📦 Memulai batch scraping {len(urls)} URL... Target: {target} email | Quality: {quality_mode}", urls, target=target, input_text=f"{len(urls)} URL dari file .txt", quality_mode=quality_mode)
            return
    ...
    if actual_mode == "batch":
        ...
        if not urls:
            ...
            return
        await run_scraping_and_send(update, context, scrape_batch, "web", actual_mode, f"📦 Memulai batch... Target: {target} email | Quality: {quality_mode}", urls, target=target, input_text=f"{len(urls)} URL manual", quality_mode=quality_mode)

    elif actual_mode == "web":
        main_input, target, quality_mode = parse_target_from_input(user_input)
        url = normalize_url(main_input)
        if not is_valid_url(url):
            await update.message.reply_text("URL tidak valid. Contoh: https://example.com | 300 | accurate", reply_markup=main_menu())
            return
        await run_scraping_and_send(update, context, scrape_website, "web", actual_mode, f"🌐 Memulai scraping web... Target: {target} email | Quality: {quality_mode}", url, target=target, input_text=url, quality_mode=quality_mode)
        ...
        if not SERPAPI_KEY:
            ...
            return
        main_input, target, quality_mode = parse_target_from_input(user_input)
        await run_scraping_and_send(update, context, scrape_maps, "maps", actual_mode, f"🗺️ Memulai Maps scraping... Target: {target} email | Quality: {quality_mode}", main_input, target=target, input_text=main_input, quality_mode=quality_mode)
        ...
        if not SERPAPI_KEY:
            ...
            return
        main_input, target, quality_mode = parse_target_from_input(user_input)
        await run_scraping_and_send(update, context, scrape_dork, "dork", actual_mode, f"🔍 Memulai Google Dorking... Target: {target} email | Quality: {quality_mode}", main_input, target=target, input_text=main_input, quality_mode=quality_mode)

    elif actual_mode == "directory":
        main_input, target, quality_mode = parse_target_from_input(user_input)
        await run_scraping_and_send(update, context, scrape_directory, "directory", actual_mode, f"📋 Memulai Direktori scraping... Target: {target} email | Quality: {quality_mode}", main_input, target=target, input_text=main_input, quality_mode=quality_mode)


async def run_scraping_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE, scrape_fn, scraping_type: str, actual_mode: str, p_msg_text: str, *args, **kwargs) -> None:
    p_msg = await update.message.reply_text(p_msg_text, reply_markup=cancel_markup())
    output_filename = get_selected_output_filename(context, scraping_type)
    target = kwargs.get("target", DEFAULT_TARGET_EMAILS)
    quality_mode = kwargs.get("quality_mode", DEFAULT_QUALITY_MODE)
    input_text = kwargs.get("input_text", str(args[0]) if args else "")
    rows = []
    start_time = time.time()

    register_job(p_msg, actual_mode, input_text, target, output_filename, quality_mode)
    save_last_job_state(actual_mode, input_text, target, output_filename, quality_mode)

    try:
        rows = await scrape_fn(*args, p_msg, target_emails=target, output_filename=output_filename, quality_mode=quality_mode)
        job_key = job_key_from_message(p_msg)
        cancelled = job_key in CANCELLED_JOBS
        if not rows:
            finish_job(p_msg)
            await safe_edit_message(p_msg, "⛔ Dihentikan." if cancelled else "✅ Selesai, tidak ada email valid baru.", reply_markup=main_menu())
            context.user_data.clear()
            return

        filepath, added, total = append_to_master_excel(rows, scraping_type, output_filename)
        elapsed = format_duration(time.time() - start_time)
        preview = "\n".join([f"{i+1}. {str(row[2 if scraping_type == 'web' else 5 if scraping_type == 'maps' else 3 if scraping_type == 'dork' else 4])}" for i, row in enumerate(rows[:10])])
        status_title = "⛔ Scraping dihentikan, hasil sementara disimpan." if cancelled else "✅ Scraping selesai."
        await update.message.reply_text(
            f"{status_title}\n\n"
            f"Mode: {actual_mode}\n"
            f"Input: {input_text}\n"
            ...
            f"Durasi: {elapsed}\n"
            f"Target: {target}\n"
            f"Baru ditambahkan: {added}\n"
            f"Total di file: {total}\n"
        ...
        await update.message.reply_document(
            ...
            caption=f"{status_title}\nBaru: {added} | Total: {total} | Durasi: {elapsed}",
        )
    except Exception as e:
        logging.exception(e)
        job_key = job_key_from_message(p_msg)
        rows = ACTIVE_JOBS.get(job_key, {}).get("rows", rows)
        saved_info = "Tidak ada hasil sementara."
        if rows:
            try:
                filepath, added, total = append_to_master_excel(rows, scraping_type, output_filename)
                saved_info = f"Hasil sementara disimpan: {os.path.basename(filepath)} | Baru: {added} | Total: {total}"
                await update.message.reply_document(document=open(filepath, "rb"), filename=os.path.basename(filepath), caption="⚠️ Hasil sementara karena error")
            except Exception as save_err:
                saved_info = f"Gagal menyimpan hasil sementara: {save_err}"
        await update.message.reply_text(
            "⚠️ Bot mengalami error, tapi proses sudah diamankan.\n\n"
            f"Mode: {actual_mode}\n"
            f"Input: {input_text}\n"
            f"Error: {e}\n"
            f"{saved_info}\n\n"
            "Cek terminal untuk traceback lengkap.",
            reply_markup=main_menu(),
        )
    finally:
        finish_job(p_msg)
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # /stop menghentikan semua job aktif pada chat ini.
    chat_id = update.effective_chat.id if update.effective_chat else None
    hit = 0
    for key in list(ACTIVE_JOBS.keys()):
        if chat_id is not None and key.startswith(f"{chat_id}:"):
            CANCELLED_JOBS.add(key)
            hit += 1
    await update.message.reply_text(f"⛔ Stop dikirim untuk {hit} proses aktif.", reply_markup=main_menu())
    ...
    try:
        if isinstance(update, Update) and update.effective_chat:
            await context.bot.send_message(
                ...
                text="⚠️ Bot mengalami error, tapi tidak dimatikan.\n\n"
