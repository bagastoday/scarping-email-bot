...
# Blokir email pemerintah dan noise kampus/akademik seperti rektor/rektorat.
# Catatan: .go.id tetap diblokir lewat BLOCKED_EMAIL_DOMAIN_SUFFIXES.
BLOCKED_GOV_DOMAIN_KEYWORDS = (
    "kemen", "kemendikbud", "kemdikbud", "kominfo", "dukcapil",
    "bappeda", "bkpsdm", "dprd", "pemkab", "pemkot", "pemprov",
    "setda", "sekda", "polri", "kejaksaan", "mahkamahagung",
)

BLOCKED_ACADEMIC_LOCAL_KEYWORDS = (
    "rektor", "rektorat", "sekretariat", "akademik", "kemahasiswaan",
    "mahasiswa", "student", "alumni", "kampus", "fakultas", "dekan",
    "perpus", "perpustakaan", "bkk", "prakerin", "magang",
)
...
def is_blocked_government_or_academic_email(local_part: str, domain: str) -> bool:
    """
    Filter tambahan:
    - blokir email pemerintah (.go.id, .gov, .mil, dan keyword instansi)
    - blokir noise akademik seperti rektor/rektorat/sekretariat/akademik
    """
    local_part = (local_part or "").strip().lower()
    domain = (domain or "").strip().lower()
    normalized = local_part.replace(".", "").replace("_", "").replace("-", "")

    if domain.endswith((".go.id", ".gov", ".mil")):
        return True

    if any(kw in domain for kw in BLOCKED_GOV_DOMAIN_KEYWORDS):
        return True

    if any(kw in local_part for kw in BLOCKED_ACADEMIC_LOCAL_KEYWORDS):
        return True

    if any(kw in normalized for kw in BLOCKED_ACADEMIC_LOCAL_KEYWORDS):
        return True
...
def validate_email(email: str) -> str:
    ...
    if is_blocked_government_or_academic_email(local_part, domain):
        return "Invalid"
...
def format_duration(seconds: float) -> str:
    ...
    if hours:
        return f"{hours}j {minutes}m {sec}d"
    if minutes:
        return f"{minutes}m {sec}d"
    return f"{sec}d"


def format_eta(start_time: float, percent: int) -> str:
...
def remember_error(counters: dict, error: str | None, url: str | None = None) -> None:
    if not counters or not error:
        return

    msg = str(error).strip()
    if url:
        msg = f"{msg} | {str(url)[:80]}"

    counters["errors"] = counters.get("errors", 0) + 1
    counters["last_error"] = msg[:250]


def format_error_info(counters: dict) -> str:
    if not counters:
        return "⚠️ Error: 0"

    total_error = counters.get("errors", 0)
    last_error = counters.get("last_error", "-")

    if total_error:
        return f"⚠️ Error: {total_error} | Terakhir: {last_error}"

    return "⚠️ Error: 0"


def should_update_progress(last_update_time: float, force: bool = False) -> bool:
    return force or (time.time() - last_update_time) >= PROGRESS_MIN_INTERVAL
    ...
    for filename in sorted(os.listdir(OUTPUT_DIR)):
        ...
        try:
            ...
            try:
                ...
                logging.warning(f"Gagal baca XLSX {path}: {e}")
                rows_count = 0
            ...
            logging.warning(f"Gagal proses file XLSX {filename}: {e}")
            continue
...
async def safe_edit_message(message, text: str, reply_markup=None) -> None:
    """
    Edit pesan Telegram dengan aman:
    - tidak memakai Markdown agar URL/query tidak bikin Bad Request
    - memotong pesan agar tidak melewati limit Telegram
    - menampilkan warning di terminal jika gagal
    """
    text = str(text)
    ...
    try:
        ...
        logging.warning(f"Koneksi Telegram bermasalah saat edit pesan: {e}")
...
async def scan_single_url(
    ...
    while queue and pages_scanned < max_pages:
        ...
        if not html:
            remember_error(counters, error or "Gagal fetch halaman", current_url)
...
async def scrape_website(
        ...
        while queued_urls:
            ...
            if scanned_pages == 1 or scanned_pages % PROGRESS_UPDATE_EVERY == 0 or not queued_urls:
                await safe_edit_message(progress_message,
                    ...
                    f"⏱️ Jalan: {format_duration(time.time() - start_time)} | ETA: {format_eta(start_time, pct)}\n"
                    f"{format_error_info(counters)}\n\n"
    ...
    await safe_edit_message(
        ...
        f"Durasi: {format_duration(time.time() - start_time)}\n"
        f"{format_error_info(counters)}\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if (doc.file_name or "").lower().endswith(".txt") and actual_mode == "batch":
            ...
            try:
                ...
                if not rows:
                    await safe_edit_message(
                        p_msg,
                        "Selesai, tidak ada email valid baru.",
                        reply_markup=main_menu()
                    )
                    context.user_data.clear()
                    return
                ...
                await safe_edit_message(
                    p_msg,
                    "❌ Bot error saat batch scraping.\n\n"
                    f"Tipe: {type(e).__name__}\n"
                    f"Pesan: {str(e)[:1500]}\n\n"
                    "Cek terminal untuk traceback lengkap.",
                    reply_markup=main_menu()
                )
                await update.message.reply_text("Mau coba lagi?", reply_markup=main_menu())
    ...
    async def _run_and_send(scrape_fn, scraping_type: str, p_msg_text: str, *args, **kwargs):
        ...
        try:
            ...
            logging.exception(e)
            error_text = (
                "❌ Bot error saat scraping.\n\n"
                f"Tipe: {type(e).__name__}\n"
                f"Pesan: {str(e)[:1500]}\n\n"
                "Cek terminal untuk traceback lengkap."
            )
            await safe_edit_message(p_msg, error_text, reply_markup=main_menu())
            await update.message.reply_text("Mau coba lagi?", reply_markup=main_menu())
        context.user_data.clear()
